...
* **プロセッサー:** 64ビットの x86 プロセッサーであれば動作します。Tattoo You は基本命令セット向けにビルドされているため AVX / AVX2 を必要とせず、一部のニューラルアンププラグインが起動しない古いマシンでも動作します。専用グラフィックカードは不要です。
* **対応OS:**
...
* **動作が重いと感じたら:** 画面下部にCPUインジケーターがあります。クリックすると、余裕があるかどうかと対処方法が表示されます。多くの場合はホスト側のバッファサイズを上げれば解決します。
